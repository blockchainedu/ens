...
describe('FIFSRegistrar', function() {
    ...
    it('registers names', function(done) {
        async.series([
            function(done) {
                registrar.register(web3.util.sha3('eth'), accounts[0], {from: accounts[0]}, done);
            },
            function(done) {
    ...
    it('forbids transferring names within the test period', function(done) {
        async.series([
            function(done) {
                registrar.register(web3.util.sha3('eth'), accounts[1], {from: accounts[0]}, done);
            },
            function(done) {
                registrar.register(web3.util.sha3('eth'), accounts[0], {from: accounts[0]}, function(err, txid) {
                    assert.ok(err, err);
                    done();
                });
            }],
